...
async function processActivity(
  ...
  try {
    ...
    const match = await findMatchingWorkout(userId, activityDate, workoutType, stravaActivityId);
    if (!match) {
      return {
        success: true,
        message: `No matching workout found for activity ${stravaActivityId} (${activity.type} on ${activityDate.toDateString()})`
      };
    }

    // Mark workout as completed
    await markWorkoutCompleted(match.workoutId, stravaActivityId, activity);

    return {
      success: true,
      message: `Workout ${match.workoutId} auto-completed from Strava activity ${stravaActivityId}`
    };
  } catch (error: any) {
