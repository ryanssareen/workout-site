...
export function ReportContainer({ report, userName, userEmail }: ReportContainerProps) {
  ...
  const getFilenameBase = () =>
    `CoachTrack-${report.title.replace(/[^a-z0-9]/gi, '-')}-${format(generatedAt, 'yyyy-MM-dd')}`;
    ...
    try {
      ...
      const res = await fetch('/api/reports/email', {
        ...
        body: JSON.stringify({
          ...
          toEmail: userEmail,
          subject: `${report.title} - CoachTrack Report`,
        }),
      });
...
Generated by CoachTrack - ${timeLabel}
    `.trim();
  ...
  return (
    <div>
      ...
      <div
        ...
        className="bg-gradient-to-br from-red-50 via-white to-white dark:from-[#0a0a0f] dark:via-[#0d0d14] dark:to-[#0b0b10] text-slate-900 dark:text-slate-100 p-8 rounded-2xl border border-red-100/60 dark:border-red-900/40 shadow-[0_20px_80px_-50px_rgba(239,68,68,0.8)] print:shadow-none print:border-0"
      >
        {/* Header with CoachTrack Branding */}
        <div className="border-b-2 border-primary pb-6 mb-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="h-12 w-12 rounded-xl bg-gradient-to-br from-red-600 to-orange-500 flex items-center justify-center shadow-lg shadow-red-500/30">
                <Dumbbell className="h-6 w-6 text-white" />
              </div>
              <div>
                <h1 className="text-2xl font-bold bg-gradient-to-r from-red-500 via-red-600 to-orange-400 bg-clip-text text-transparent drop-shadow">
                  CoachTrack
                </h1>
        ...
        <div className="mt-12 pt-6 border-t border-slate-200 dark:border-slate-800 text-center">
          <p className="text-xs text-slate-400 dark:text-slate-600">
            {report.footer || `Generated by CoachTrack | ${timeLabel}`}
          </p>
        </div>
      </div>
    </div>
